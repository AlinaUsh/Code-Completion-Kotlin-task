...
class Lexer(Iterable[Token]):
    ...
    def __init__(self, data: str, yield_comments: bool = False) -> None:
        self.data = data
    ...
    def __iter__(self) -> Iterator[Token]:
        ...
        while self.i < self.length:
            ...
            elif self.i + 2 < self.length and c_start + self.data[self.i + 2] == "/**":
                yield self._read_docstring()
    ...
    def _read_docstring(self) -> DocstringToken:
        i = self.i + 3
        count = 1
        while i < self.length and count > 0:
            c = self.data[i]
        ...
        return self._read_token(DocstringToken, i)
